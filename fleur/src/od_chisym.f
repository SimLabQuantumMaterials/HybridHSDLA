...
c for systems without z-reflection and inversion symmetries
        DO n = 2,odd%nop
         mrot(1,1,n) =  cos(tpi*(n-1)/odd%nop)
         mrot(1,2,n) =  sin(tpi*(n-1)/odd%nop)
         mrot(2,1,n) = -sin(tpi*(n-1)/odd%nop)
         mrot(2,2,n) =  cos(tpi*(n-1)/odd%nop)
...
c for systems with inversion or z-relection symmetries
      ELSE
         ...
         IF ((odd%nop)/2. - half > 0.001) THEN
           STOP 'od_chisym: nop =/= 2*n & (invs v zrfs) !'
         ENDIF
         DO n = 2,half
            mrot(1,1,n) =  cos(tpi*(n-1)/half)
            mrot(1,2,n) =  sin(tpi*(n-1)/half)
            mrot(2,1,n) = -sin(tpi*(n-1)/half)
            mrot(2,2,n) =  cos(tpi*(n-1)/half)
         END DO 
         IF (odd%zrfs) THEN
            mrot(1,1,half+1) =  1.
            mrot(2,2,half+1) =  1.
