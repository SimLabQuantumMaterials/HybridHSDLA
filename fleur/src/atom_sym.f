...
!--> actually, intent out:
      INTEGER, POINTER :: neq(:), ntyrep(:)    ! here, these variables are allocated with
      REAL,    POINTER :: zatom(:)             ! the dimension 'ntype'
      INTEGER, POINTER :: natype(:),natrep(:),natmap(:)  ! or  'nat'
      REAL,    POINTER :: pos(:,:)                       ! or  '3,nat'
      INTEGER, POINTER :: mrot(:,:,:)                    ! or  '3,3,nop'
      REAL,    POINTER :: tau(:,:)                       ! or  '3,nop'
...
!--->    check whether this atom exists already
     ...
     &             CYCLE repres_atoms
               STOP 'ERROR! atom_sym: mismatch between atoms.'
            ENDIF
         ENDDO
...
!--->       check whether this is a new atom
     ...
     &                       " symmetry input")')
                  STOP 'atom_sym: mismatch rotated'
               ENDIF
            ENDDO
!--->       new position
...
!---> stop if input is inconsistent
     ...
     &         (imap(nt)<0),(icount(abs(imap(nt)))>1)
          ...
          STOP 'atom_sym: too many representative atoms given in input.'
