...
C     .. Local Arrays ..
      ...
      DO nk = 1,nkpt
        DO neig = nevk(nk)+1,neigd
          ev(neig,nk) = 1.0e10 
        ENDDO
        DO neig = 1,neigd
          wparint(neig,nk) = 0.0e0
          DO ntp = 1,ntype
            DO ns = 1,4
              wpar(ns,ntp,neig,nk) = 0.0e0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
...
!          DO i = 1,3
!            DO j = i+1,4
!              IF (abs(eval(i)-eval(j)).LT.1.0e-7) THEN
!                 eval(i) = eval(i) + 1.0e-7
!                 eval(j) = eval(j) - 1.0e-7
!              ENDIF
!            ENDDO
!          ENDDO
      ...
      DO nk=1,nkpt
        DO neig = 1,nevk(nk)
          DO ntp = 1,ntype
            ...
            DO ntet = 1,ntetra
              DO i = 1,4
                IF ( itetra(i,ntet).eq.nk ) GOTO 460
              ENDDO
              GOTO 530
  460         CONTINUE
              DO i = 1,4
