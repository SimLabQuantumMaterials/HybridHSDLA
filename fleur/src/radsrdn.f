...
C     ..
C     .. Array Arguments ..
      REAL,    INTENT (IN) :: f0(jmtd,2),vr(jmtd)
      REAL,    INTENT (OUT):: fn(jmtd,2),fni(jmtd,2)
C     ..
C     .. Local Scalars ..
      INTEGER i,j,k
...
C     ..
C     .. Local Arrays ..
      REAL f(jmtd,2,0:n),help(jmtd)
C
C
      IF(n.lt.1) STOP 'radsrdn: n<1!'
      f(:,:,0) = f0(:,:)
C
      DO i=1,n
C       Calculate
...
C        scprod=0 ! Instead of the nth derivative a linear combination
        ...
        CALL getfni(e,l,vr,r0,h,jri,jmtd,c,i,f,fni)
        CALL radsrdn1(
     >             e,l,vr,r0,h,jri,jmtd,c,
     <             udn,dudn,ddnn,nodedn,f(1,1,i),f(1,2,i),
     >             fni(1,1),fni(1,2),f0(1,1),f0(1,2),du0,scprod)
      ENDDO
      fn(:,:) = f(:,:,n)
      help(:) = fni(:,1)
      fni(:,1)=-fni(:,2)
...
C     ..
C     .. Array Arguments ..
      REAL,    INTENT (IN) :: pi(jmtd),qi(jmtd),vr(jmtd)
      REAL,    INTENT (OUT):: pe(jmtd),qe(jmtd)
      REAL,    INTENT (IN) :: phom(jmtd),qhom(jmtd)
C     ..
C     .. Local Scalars ..
...
c--->    test quality of corrector and iterate if necessary
            ...
            q0 = q1
         ENDDO corrector
         IF (it > 5) STOP 'radsrd: not converged.' 
c--->    store values
         pe(i+1) = p1
         qe(i+1) = q1
         pp(i+1) = p1p
...
C---------------------------------------------------------------------
      ...
      SUBROUTINE getfni(e,l,vr,r0,h,jri,jmtd,c,n,f,fni)
C*********************************************************************
...
C     ..
C     .. Scalar Arguments ..
      INTEGER, INTENT (IN) :: n,jri,jmtd,l
      REAL,    INTENT (IN) :: c,e,h,r0
C     ..
C     .. Array Arguments ..
      REAL,    INTENT (IN) :: f(jmtd,2,0:n-1),vr(jmtd)
      REAL,    INTENT (OUT):: fni(jmtd,2)
C     ..
C     .. Local Scalars ..
      INTEGER i
      REAL r
