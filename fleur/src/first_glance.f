...
c
     ...
     <                        l_kpts,l_qpts,nkpt,nmop,nqpt,nmopq)
      ...
      LOGICAL,INTENT (OUT) :: l_kpts,l_qpts
...
c
      ...
      IF (.not.l_kpts) THEN
        ...
        ENDDO
        READ (5,'(5x,i5,3(4x,i2))',END=97,ERR=97) 
...
!
      ...
      IF (n2spg == 0 ) THEN
        ...
        STOP 'first glance: could not determine spacegroup!'
      ENDIF
      IF ( (n2spg.GE.13).AND.(n2spg.LE.17) ) THEN
        IF ( .not.((latnam.EQ.'hx3').OR.(latnam.EQ.'hex')) ) THEN
          STOP 'use only hex or hx3 with p3, p3m1, p31m, p6 or p6m!'
        ENDIF
      ENDIF
...
! Error
!
  99  WRITE (6,*) 'Error glancing  at inp-file line',line
      CLOSE (6)
      STOP
